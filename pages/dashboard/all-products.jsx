...
import AllProductsDataTable from "./component/allproductstable";
...
const AllProductsPage = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Keep using mockProducts as fallback
            } finally {
                setLoading(false);
            }
        };
        ...
        switch (bulkAction) {
            case 'publish':
                setProducts(prev => prev.map(p => 
                    selectedIds.includes(p.id) ? {...p, status: 'Publish'} : p
                ));
                break;
            case 'draft':
                setProducts(prev => prev.map(p => 
    ...
    const filteredProducts = products.filter(product => {
        ...
        if (priceFilter === 'under1000') {
            matchesPrice = (product.price || 0) < 1000;
        } else if (priceFilter === '1000-5000') {
            matchesPrice = (product.price || 0) >= 1000 && (product.price || 0) <= 5000;
        } else if (priceFilter === 'over5000') {
            matchesPrice = (product.price || 0) > 5000;
        }
